...
let ExampleService = class ExampleService {
    ...
    async getlist() {
        return await this.sql.query(`SELECT * FROM asdasd`);
    }
    async transactionTest() {
        return await this.sql.transaction([
            `UPDATE adminuser SET username = 'leading' WHERE id = 4`,
            `INSERT INTO transactionT(name,password,age) VALUES ('admin','123123','25')`
        ]);
    }
};
