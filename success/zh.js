...
//进程参数
const ACCOUNT = process.argv[2]; //  process.argv[2];
...
if (ACTION == 'pay') {
  ...
  ISCHECK = process.argv[9];
} else {
  QUOTA = process.argv[5];
  CHANNEL = process.argv[6];
  var ZID = process.argv[7];
...
deleteFolder(path.join(USERDATADIR, `${ACCOUNT}`));
async function main() {
  let arg = {
    headless: HEADLESS,
  ...
  intervalId = setInterval(async () => {
    ...
    if (!ISCHECK) {
      ...
      if (ISCHECK) {
        process.send({ action: 'check', result: '失败', zh: ACCOUNT });
      }
      if (runtime > 30) {
        outime = true
      }
  ...
  QBcheckpay: async function () {
    ...
    for (let index = 0; index < tframe.length; index++) {
      if (tframe.length >= 7) {
        ...
        if (content.indexOf('fpm-ui-cell-sub-txt') > -1) {
          //console.log("QB余额元素");
          await p.findClick(tframe[index], `.fpm-ui-cell-sub-txt`);
        } else if (content.indexOf('fpm-ui-cell-primary') > -1) {
          if (ISCHECK) {
            process.send({ action: 'check', result: '失败', zh: ACCOUNT });
          }
        }
  ...
  clickchangeVerfiy: async function () {
    ...
    if (tframe3.length > 6 && viewpass) {
      ...
      if (!ISCHECK) {
        ...
        console.log("能正常到安全码界面");
        if (ISCHECK) {
          process.send({ action: 'check', result: '成功', zh: ACCOUNT });
        }
      }
        ...
        case "DNF"://template
          switch (sai) {
            case 0:
              ...
              let isnext = await this.gotoInputAccount(DNFurl, input);
              if (isnext) {
                await sleep(3000);
              }
              break;
            case 1:
              ...
              let isindex = await p.find(dianjuan)
              if (!isindex) {
                sai--;
              }
              break
        ...
        case "LOL":
          ...
          switch (sai) {
            ...
            case 1://开始选区
              ...
              if (lilist && lilist.length > 15) {
                sai++;

              } else if (lilist && lilist.length == 0) {
                sai--;
              }
        ...
        case "CF":
          ...
          switch (sai) {
            ...
            case 1:
              ...
              let isindex2 = await p.find(CFdianjuan)
              if (!isindex2) {
                sai--;
              }
              break
